...
/* OVERVIEW:
    --> DataBoard1 is a container for storing and displaying generic datas which extend type Data1. It guarantees privacy of datas, allowing modification only to
        the owner and viewing to only added friends (to each category). Moreover it allows added friends to give likes to datas.
    --> TYPICAL ELEMENT: <password, <data1, data2, ......, data.size()>, <category1, category2, ......, category.size()>>
    --> REPRESENTATION INVARIANT: this.password != null && this.password.isEmpty == false
                  for each "i" in datas.size(): this.datas.get(i) != null &&
                                                this.datas.get(i).getName() != null &&
                                                this.datas.get(i).getName().isEmpty == false &&
                                                this.datas.get(i).getCategory() != null &&
                                                this.datas.get(i).getCategory().getName().isEmpty() == false &&
                                                this.datas.get(i).getLikes() >= 0 &&
                                                for each "a" in datas.size(): this.datas.get(i).getName().equals(this.datas.get(a).getName()) == false
           for each "j" in myCategories.size(): this.myCategories.get(j) != null &&
                                                this.myCategories.get(j).getName() != null &&
                                                this.myCategories.get(j).getName().isEmpty == false &&
                                                for each "b" in myCategories.size(): this.myCategories.get(j).getName().equals(this.myCategories.get(b).getName()) == false
                                                for each "z" in friends.size(): this.myCategories.get(j).friends.get(z) != null &&
                                                              this.myCategories.get(j).friends.get(z).isEmpty == false &&
                                                              for each "c" in friends.size(): this.myCategories.get(j).friends.get(z).equals(this.myCategories.get(j).friends.get(c)) == false
    --> ABSTRACT FUNCTION: AB = [password, <datas.get(1), datas.get(2), ....., datas.get(datas.size())>, <myCategories.get(1), myCategories.get(2), ....., myCategories.get(myCategories.size())>]
*/

public interface DataBoard1<E extends Data1>{
